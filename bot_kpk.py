...
LAST_CONFESSION_TIME = load_json("last_confession_time.json")  # НОВЕ: кулдаун
...
def can_send_confession(user_id: int) -> tuple[bool, int]:
    """Перевіряє чи може користувач надіслати зізнання (кулдаун 30 хв)"""
...
async def handle_user_message(message: types.Message):
    ...
    if action == "confession":
        # ✅ Перевірка кулдауну для зізнань
        ...
        LAST_CONFESSION_TIME[user_id_str] = time.time()  # ✅ Оновлюємо час
        save_all()
        
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Одобрити", callback_data=f"approve:{conf_id}"),
                InlineKeyboardButton(text="❌ Відхилити", callback_data=f"reject:{conf_id}"),
            ],
            [
                InlineKeyboardButton(text="🔨 Блок 1 год", callback_data=f"block:{conf_id}:3600"),
                InlineKeyboardButton(text="🔨 Блок 24 год", callback_data=f"block:{conf_id}:86400"),
            ],
            [InlineKeyboardButton(text="🔨 Блок назавжди", callback_data=f"block:{conf_id}:permanent")]
        ])
        
        await bot.send_message(
            ADMIN_CHAT_ID,
            f"💌 Нове анонімне зізнання:\n\n{message.text}\n\n🕐 Останнє: {time.strftime('%H:%M', time.localtime(LAST_CONFESSION_TIME[user_id_str]))}",
            reply_markup=kb,
        )
        await message.answer(
            "Дякуємо! Твоє повідомлення надіслано на модерацію 💌\n⏰ Наступне через 30 хв",
            reply_markup=main_kb,
        )
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="💬 Відповісти", callback_data=f"support_reply:{conf_id}"),
                InlineKeyboardButton(text="❌ Відхилити", callback_data=f"support_reject:{conf_id}"),
            ],
            [
                InlineKeyboardButton(text="🔨 Блок 1 год", callback_data=f"block:{conf_id}:3600"),
                InlineKeyboardButton(text="🔨 Блок 24 год", callback_data=f"block:{conf_id}:86400"),
        ...
        await message.answer("Ваше звернення до техпідтримки надіслано.", reply_markup=main_kb)
...
@dp.callback_query(lambda c: c.data and c.data.startswith(("support_reply:", "support_reject:")))
async def process_support(call: types.CallbackQuery):
    ...
    if action == "support_reply":
        ...
        try:
            await bot.send_message(data["user_id"], "Ваше звернення техпідтримки відхилено ❌")
        except:
            pass
        await call.answer("❌ Відхилено")
...
async def moderator_reply(message: types.Message):
    if not message.reply_to_message:
        await message.reply("Будь ласка, відповідайте на повідомлення бота з техпідтримкою.")
        return
    ...
    if not key_id:
        await message.reply("Це повідомлення не містить відкритого звернення техпідтримки.")
        return
    ...
    try:
        await bot.send_message(user_id, f"💬 Відповідь техпідтримки:\n\n{message.text}")
